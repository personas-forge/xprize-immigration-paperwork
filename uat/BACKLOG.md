...
> **Progress — Tier-2 COMPLETE:** LLM-1 interim caveat → **#97**; LLM-1 model-backed best-path → **#98**
> (live-verified — a director is now recommended O-1B with cross-classification reasoning); LLM-2
> grounding gate → **#99**; LLM-4 strong-but-unscored nudge → **#100**; **LLM-3 grounding floor / domain
> hints → this PR**. Next: **Tier 3** (positioning T5, SEO professions, ops read-only role, the real
> FAQ-security project).
