...
2. **Discover the run recipe → `env.md`.** From `package.json` scripts (or Makefile/Procfile): dev/start command + port — **and a free-port fallback ladder**, because the default port is routinely already taken by a parallel UAT session on another repo (see *Driver & environment → Server lifecycle & port contention*). Detect the framework, the UI **language**, any **auth** + whether a dev bypass exists, and whether authed data can be served locally (local DB / seed / fixture). **An offline auth + local-data path is the single biggest unlock for authed coverage** — recommend or wire one if missing.
...
- **Server lifecycle & port contention:** reuse an already-running server *for this repo* (don't start a 2nd instance) — but never assume the env's default port is yours to take. UAT is run as periodic sweeps and **several repos are routinely under `/uat` at once**, so the conventional dev port (3000, 5173, 8080…) is frequently already held by *another project's* server. **Preflight the port:** probe the env default; if it's free, use it; if something answers, confirm it's *this* repo's app (expected title/route/health) — if it's a foreign app (or the port refuses), **step to the next free port on the env's fallback ladder**, start the dev server bound to it (`PORT=…` / `-p` / `--port`, per the stack), and thread that port through `BASE_URL` **and every driver invocation**. **Record the chosen port** in `runs/<id>/.port` so every driver in the run — and any later `recertify` — targets the same instance. A driver pointed at the wrong port silently tests a neighbor's app, so **assert server identity before trusting any result**. **Recover a wedged server** (hangs / `ECONNREFUSED` / bundler cache errors like Turbopack "corrupted database", often after a `git checkout` swapped files under it): kill the port, delete the build cache (`.next`/`.vite`), restart, re-poll.
...
- **Port contention across parallel sessions** — treat the env's port as a *default, not a reservation*. With several repos under `/uat` simultaneously they collide on the conventional port; detect-and-step to a free port (above), record it per-run (`runs/<id>/.port`), and isolate any side state (build-cache dir, local DB path) if two instances of the *same* repo could ever run at once. Always assert the server you reached is yours before recording findings against it.
...
1. Drop `/uat` into the repo (`.claude/skills/uat.md` — it's a local, copy-to-other-repos asset). 2. `/uat init` → discovers routes/run-recipe/auth/language, researches, **asks how many Characters (1/5/10)**, scaffolds `uat/`. 3. Resolve env open-questions (esp. offline auth + local-data, and the **free-port fallback ladder**). 4. `/uat run --l1` for a cheap broad sweep; then `/uat run` for full L1→L2. 5. Fix, then **`/uat recertify`** to re-verify just the touched journeys (resolved-verified + metric deltas). 6. `/uat promote` clean journeys into gates.
