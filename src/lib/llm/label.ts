 ...
 */
import type { LlmEngine } from "./config";

/** Where an AI payload came from: a template fallback, or a model engine. Derived
 *  from the canonical `LlmEngine` union so the engine names live in one place. */
export type ModelSource = LlmEngine | "mock";
...
export function isModelSource(source: string): source is LlmEngine {
